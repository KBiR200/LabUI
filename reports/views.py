...
def tasks(request):
    ...
    Q(creator__team_members__members=request.user) | Q(assigned=request.user)
        ).prefetch_related('creator__team_members__members').distinct().order_by('-created_at')
    # team = Team.objects.filter(members=request.user)
    # new_tasks = Tasks.objects.filter(
    # Q(status=0) &
    # Q(creator__team_members__members=request.user)
    #     ).prefetch_related('creator__team_members__members').distinct().order_by('-created_at')
...
def show_report(request, pk):
    ...
    print(rep.task.creator)
    if  not (request.user.id in list(rep.author.values_list("id", flat=True)) or not ( request.user.id != rep.task.creator.id)):
